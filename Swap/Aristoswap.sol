...
import {Swap, AssetType} from "./SwapStruct.sol";
...
contract Aristoswap is Ownable {
    ...
    constructor(
        address[] memory _projectCollections,
        address _daoWallet,
    ...
    function withelistCollections(
        ...
        for (uint256 i = 0; i < collections.length; i++) {
            ...
            require(
                collectionAllowed[collection] == false,
    ...
    function createSwap(
        Swap calldata swapMaker,
        Swap calldata swapTaker,
        address feeToken
    ) external payable {
        if (msg.sender != swapMaker.trader) revert WrongCaller();

        if (_validateFees(feeToken, swapMaker.croAmount) == false)
            revert FeesNotPaid();
